          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800/30 p-5 rounded-lg border border-slate-100 dark:border-slate-800">
              ...
              <div className="md:col-span-2">
                ...
                <input type="text" required placeholder="e.g. Premium Basmati Rice" className="input-field" value={productName} onChange={(e) => setProductName(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Brand</label>
                <input type="text" placeholder="e.g. India Gate" className="input-field" value={brand} onChange={(e) => setBrand(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Category</label>
                <select required className="input-field appearance-none" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>
                  {categories.map((c) => (
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Stock Unit</label>
                <input type="text" required className="input-field" placeholder="e.g. piece, kg, pack" value={unit} onChange={(e) => setUnit(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Transit Time (Days)</label>
                <input type="number" required min="1" className="input-field" value={leadTimeDays} onChange={(e) => setLeadTimeDays(e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-800/30 p-5 rounded-lg border border-slate-100 dark:border-slate-800">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Trade Price (₹)</label>
                <input type="number" step="0.01" required className="input-field text-primary-600 font-semibold" value={wholesalePrice} onChange={(e) => setWholesalePrice(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">MRP (₹)</label>
                <input type="number" step="0.01" className="input-field font-medium" value={mrp} onChange={(e) => setMrp(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Min Order Qty</label>
                <input type="number" required min="1" className="input-field font-medium" value={minOrderQty} onChange={(e) => setMinOrderQty(e.target.value)} />
              </div>

              <div className="md:col-span-3">
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Initial Stock Level</label>
                <input type="number" required min="0" className="input-field font-medium" value={availableStock} onChange={(e) => setAvailableStock(e.target.value)} />
              </div>
            </div>
          </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/30 p-5 rounded-lg border border-slate-100 dark:border-slate-800 flex-1 flex flex-col">
              ...
              <div className="mt-6 space-y-3">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full btn-primary py-2.5 text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-2 group"
                >
