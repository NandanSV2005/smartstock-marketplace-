    ...
    <div className="flex flex-wrap gap-1.5 items-center">
     ...
     <div className="relative z-50">
      <button
       ...
       className="px-3 py-1.5 text-sm font-medium rounded transition-all flex items-center bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-600 dark:hover:bg-slate-700  relative"
      >
      ...
      {showNotifications && (
       <div className="absolute right-0 mt-2 w-80 rounded-md shadow-lg z-50 border border-slate-200 overflow-hidden text-left bg-slate-100">
        ...
        <div className="max-h-96 overflow-y-auto custom-scrollbar">
         {notificationsState.data.length === 0 ? (
          ...
          notificationsState.data.slice(0, 20).map(note => (
           <div key={note.id} className={`p-4 border-b border-slate-200 cursor-pointer transition-all hover:bg-slate-50 dark:hover:bg-slate-850 ${note.read_at ? 'opacity-60 bg-transparent' : 'bg-primary-50/50 dark:bg-primary-950/10'}`} onClick={() => !note.read_at && handleMarkNotificationRead(note.id)}>
            <h4 className="text-sm font-semibold text-slate-800 mb-1">{note.title}</h4>
            <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{note.body}</p>
    ...
    <div className="dash-section p-6 border-orange-200 dark:border-orange-500/20 bg-orange-50/20 dark:bg-orange-950/5 shadow-sm shadow-tactile-primary relative">
     <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div className="flex items-center gap-3">
       ...
       <div>
        <h3 className="text-lg font-semibold text-slate-800">Supplier Marketplace</h3>
     ...
     <div className="p-6">
      {cart.loading ? (
       ...
       <div className="text-center py-24">
        ...
        <h4 className="text-lg font-semibold text-slate-800">Your cart is empty</h4>
         ...
         {inventory.data.filter(item => Number(item.current_stock) > 0).length === 0 ? (
          <tr>
           <td colSpan={5} className="px-6 py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-700">No Active Inventory</p>
           ...
           <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
            {editingInventoryId === item.id ? (
             <div className="flex items-center space-x-2">
              <input 
               type="number"
               className="w-16 bg-slate-100 border border-slate-200 rounded px-2 py-1 text-sm font-medium text-slate-800 focus:border-primary-500 focus:outline-none"
         ...
         <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pl-2">
          ...
          <div className="flex flex-wrap items-center gap-8 lg:gap-12">
           <div className="text-right">
            ...
            <span className="text-sm font-semibold text-slate-800">₹{order.total_amount}</span>
           </div>
           <div className="text-right">
