...
export function WholesalerDashboard() {
 ...
 return (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
   <div className="mb-8 pb-4 border-b border-slate-200 flex flex-col md:flex-row md:items-end justify-between gap-6">
    <div>
       ...
       <div className="lg:col-span-2 space-y-6">
        ...
        <section className="dash-section p-6">
         <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center justify-center w-6 h-6 rounded bg-slate-100 text-xs text-slate-600 font-medium">
           <span>{pendingOrders.length}</span>
          </div>
          <h3 className="text-base font-semibold text-slate-800">New Orders</h3>
         </div>
            ...
            <li key={order.id} className="card p-4 flex justify-between items-center group hover:border-slate-300 dark:hover:border-slate-700 transition-all animate-scale-in">
             <div>
              <div className="flex items-center space-x-3 mb-1">
               <h4 className="font-semibold text-slate-800 text-sm">Order #{order.order_number}</h4>
              ...
              <p className="text-xs text-slate-500 mt-1 max-w-xs truncate" title={order.delivery_address}>{order.delivery_address}</p>
             </div>
        ...
        <section className="dash-section p-6">
         <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-200">
          ...
          <h3 className="text-base font-semibold text-slate-800">Ongoing Deliveries</h3>
         </div>
        ...
        <section className="dash-section p-6 flex flex-col h-full">
         <div className="flex justify-between items-center mb-6 pb-3 border-b border-slate-200">
          <div className="flex items-center gap-3">
           ...
           <h3 className="text-sm font-semibold text-slate-800">Your Products</h3>
          </div>
          <span className="text-xs bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-slate-600 font-medium">{products.length} items</span>
         </div>
             ...
             <li key={p.id} className="py-4 first:pt-0">
              {editingProductId === p.id ? (
               <div className="space-y-4 p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 animate-scale-in">
                <p className="text-xs font-semibold text-slate-800 truncate block border-b border-slate-100 dark:border-white/5 pb-2">{p.product.name}</p>
               ...
               <div className="flex justify-between items-start group relative">
                <div className="flex-1 min-w-0">
                 <p className="text-sm font-semibold text-slate-800 truncate pr-4 group-hover:text-primary-650 transition-colors">{p.product.name}</p>
     ...
     {activeTab === 'ledger' && !loading && (
      <div className="animate-fade-in space-y-6">
        ...
        {receivablesLoading ? (
         ...
         <>
          <div className="kpi-card">
           <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 flex items-center justify-center text-blue-600 mb-3">
           ...
           <span className="stat-value text-slate-800">₹{receivables.total_pending_amount.toLocaleString()}</span>
           <p className="text-xs text-slate-500 mt-1">Outstanding across all retailers</p>
          </div>
          <div className="kpi-card">
           <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 flex items-center justify-center text-orange-500 mb-3">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
           </div>
           <span className="text-xs text-slate-500 font-medium block mb-1">Credit Orders</span>
           <span className="stat-value text-slate-800">{receivables.total_credit_orders}</span>
           <p className="text-xs text-slate-500 mt-1">Open credit obligations</p>
          </div>
          <div className="kpi-card">
           <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 flex items-center justify-center text-red-500 mb-3">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
           </div>
           <span className="text-xs text-slate-500 font-medium block mb-1">Overdue Count</span>
           <span className={`stat-value ${receivables.overdue_count > 0 ? 'text-red-500' : 'text-slate-800'}`}>{receivables.overdue_count}</span>
           <p className="text-xs text-slate-500 mt-1">Payments past due date</p>
          </div>
          <div className="kpi-card">
           <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 flex items-center justify-center text-amber-500 mb-3">
           ...
           <span className={`stat-value ${receivables.overdue_amount > 0 ? 'text-amber-600' : 'text-slate-800'}`}>₹{receivables.overdue_amount.toLocaleString()}</span>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="kpi-card">
         <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 flex items-center justify-center text-emerald-500 mb-3">
       ...
       <section className="dash-section p-6">
        <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-200">
         ...
         <h3 className="text-base font-semibold text-slate-800">Retailer Payment History</h3>
        </div>
            ...
            orders.filter(o => Number(o.amount_due) > 0).map(order => {
             ...
             return (
              <tr key={order.id} className={isOverdue ? 'bg-red-50/20 dark:bg-red-950/10' : ''}>
               ...
               <td>
                <span className="font-medium text-slate-700">₹{Number(order.total_amount).toFixed(2)}</span>
               </td>
               <td>
      ...
      <div className="animate-fade-in space-y-6">
       <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
         <div className="w-1 h-6 bg-primary-600 flex-shrink-0" />
         <div>
          <h3 className="text-lg font-semibold text-slate-800">Retailer Credit Overview</h3>
          <p className="text-xs text-slate-500 mt-0.5">Scores based on how quickly retailers pay their bills.</p>
         </div>
        </div>
        <button
            ...
            {creditProfiles.map(profile => (
             <tr key={profile.retailer_id} className={profile.risk_level === 'high' ? 'bg-red-50/10 dark:bg-red-950/10' : ''}>
              <td className="px-4 py-3">
               <div>
                <p className="font-semibold text-slate-800 text-sm">{profile.retailer_name}</p>
                <p className="text-xs text-slate-500">{profile.business_type}</p>
               </div>
              </td>
              <td className="px-4 py-3">
               <div className="flex items-center gap-3">
                <div className="flex-1 bg-slate-200 rounded-full h-1.5 w-20 overflow-hidden">
                 <div
                ...
                <span className="text-sm font-semibold text-slate-800 tabular-nums">{profile.credit_score}</span>
               </div>
              </td>
              <td className="px-4 py-3">{riskBadge(profile.risk_level)}</td>
              <td className="px-4 py-3">
               <span className="text-sm font-semibold text-slate-800">₹{profile.credit_limit_suggestion.toLocaleString()}</span>
              </td>
              <td className="px-4 py-3">
